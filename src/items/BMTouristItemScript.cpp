...
#include "Chat.h"
#include "../BMConfig.h"
#include "BMTouristItemScript.h"

uint32 DIVINE_SPELL_ID = 642;
uint32 DIVINE_DURATION_MS = 60000;

bool ModBeerMaster_Tourist_ItemScript::OnUse(Player *player, Item *item, const SpellCastTargets &)
{
    if (!modBmEnabled)
        return false;
...
void ModBeerMaster_Tourist_ItemScript::Teleport(Player *player, GameTele const *tele)
{
    if (!player->IsInWorld())
        return;
...
void ModBeerMaster_Tourist_ItemScript::AfterTeleport(Player *player)
{
    if (!player->IsInWorld())
        return;
...
GameTele const *ModBeerMaster_Tourist_ItemScript::GetTeleportLocation()
{
